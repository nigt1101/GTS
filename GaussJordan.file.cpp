...
// Đọc ma trận từ tệp văn bản
void readMatrix(vector<vector<double>>& matrix) {
    ifstream file("C:\\Users\\dell\\Downloads\\4_Seidel.txt");  // Thay đổi đường dẫn tới tệp văn bản chứa ma trận
    if (file.is_open()) {
        int rows, cols;
        file >> rows >> cols;  // Đọc số hàng và số cột của ma trận bo sung
        matrix.resize(rows, vector<double>(cols));
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                file >> matrix[i][j];  // Đọc giá trị từng phần tử của ma trận
            }
        }
        file.close();
    }
